function pattern_pool=T_FIND_PATTERNS( events, ep, Nt, pattern_window, use_cuda, conf )
%ep = ps;
%pattern_window = 250;
...
for lp_i = 1 : numel( pattern_pool )
    for rp_i = 2 : numel( pattern_pool )
        ...
        if (~isempty( intersect( pL.Events, pR.Events ) ) ) && ( ~conf.allow_same_events )
            continue
        end
        ...
        [new_pats, found_patterns] = T_ADD_PATTERN( events, ep, pL, pR, 1, pattern_window, Nt, use_cuda, conf );
        if ( found_patterns ~= 0 )
...
%             end
            for i = 1:numel(pR.Events)
                if nnz( ismember( pL.Events, pR.Events ) ) == numel( pL.Events )
                    R = pR.LhSHIFTED(i,:);
                    if (pL.Lh*R') / ( sqrt( pL.Lh*pL.Lh' ) * sqrt( R*R' ) ) >= conf.cor
                        if numel(pL.Events) > 1
                            to_del = [ to_del lp_i];
