...
% Recount sigma
% .....
CIs = [20 30; 10 15; 3 6; 5 7; 3 6; 15 22; 40 45];
Thetas = [ 0 1; 85 2; 54 2; 120 3; 82 2; 15 3];
mesh_step = 0.01;
pattern_window = 800;
[events, Nt, ts] = T_GENERATE_PATTERN('abcdefgh', 'xyzw', CIs, 8, 900, 0.01, 0.01);
%[events, Nt, ts] = T_GENERATE_FUZZY_PATTERN('abcdef', 'xyz', Thetas, 7, 400, 0.001, 0.001);
ps = T_PS_FROM_TS(events, Nt, 1);

%figure1 = figure();%'visible','off');
%T_CMP_LH( ps(1).Lh, ps(2).Lh, 500, true, false);
%saveas(gcf,'lh.eps', 'psc2')

np1=T_ADD_PATTERN(events, ps, ps(1), ps(2), mesh_step, pattern_window, Nt);
%figure1 = figure()%'visible','off');
%T_CALC_LH( events, Nt, np1, true, false);
%saveas(gcf,'lhh.eps', 'psc2')
...
% T_CMP_LH(pp1, pp2, pattern_window);
np3=T_ADD_PATTERN(events, ps, np1, np2, mesh_step, pattern_window, Nt);

np4=T_ADD_PATTERN(events, ps, np3, ps(5), mesh_step, pattern_window, Nt);
np5=T_ADD_PATTERN(events, ps, ps(5), ps(6), mesh_step, pattern_window, Nt);
np6=T_ADD_PATTERN(events, ps, np4, ps(6), mesh_step, pattern_window, Nt);
np7=T_ADD_PATTERN(events, ps, np3, np5, mesh_step, pattern_window, Nt);
T_CALC_LH(events, Nt, np3, true, false);
...
% [pLR, ddf] = T_CMP_2_PATTERNS( events(1).indexes, events(2).indexes, pattern_window);
...
%     line = ( double(maximums(i)) - dist_win * 2 ) : mesh_step : ( double(maximums(i)) + dist_win * 2 );
%     plot ( line,  -1-dens( double(maximums(i))/mesh_step )*gK*sqrt(2*pi)*sigma  * normpdf(line, double(mu), sigma), '-','Color', 'k' );
% end
% sigmas
% 
% 
...
% tic
% levels = [ 1 0.05 3];
% [events2, Nt2] = T_LOAD_FILE('../real_data/open field_rodents/SU2/su13.txt'); %'../real_data/grooming_hamsters/auratus1.txt');
